//                     CONTROL BOARD UPDATED! 15.04.21
//*****************************************************************************
// Program:	 Dimmer
// Author:	 Francis Lee
...
void SET_RGB(rValue,gVlaue,bValue);
...
static unsigned  int 			i,j,k,x,y,z,w,SV_ID[12];
static unsigned  int 		    sigDigits;
...
static unsigned int             bulbIntensity, bulbEnable, bulbMode;
static long int 				outputVal,R_Value,G_Value,B_Value;
static unsigned char			commandVal[3],bulbState;
...
void GetUART_Command(void){   
	...
	// ***************** Receive GPS into Buffer ********************
		...
		while(_flgUartFin != 1){ 
			...
			I2C_Read(0x38u, &Prox_PS_LSB, 1, uniRawSensorOut._ucharArr, 1);
			/*
			if((int)uniRawSensorOut._ucharArr[0] <250){ 
